...
3. controller(in, state): Computes the controller output based on the current state of the aircraft. A sample controller is provided. 

***** The code must be replaced with your code. ******

4. safetyMonitor(in1, in2): It returns true if the aircraft avoid collision avoidance. 
***** Add your code here: currently the value of flag is always false! ******
...
For example, say A goes from (0, 0) -> (0, 10) and B goes from (-5, 5) -> (5, 5). To run the simulation for at most 40 steps the following command is used. 
